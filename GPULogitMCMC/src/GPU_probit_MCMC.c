__global__ void LL_probit_GPU_1(int* obs, float* beta, float* data_mat, float* Log_like_GPU, int n, int p) {
...
extern "C" {
  ...
  void GPU_probit_MCMC(int* obs,
    ...
    Proposed_LL = (float*) malloc(sizeof(float));
      ...
      //Perform Computation
     LL_probit_GPU_1<<<grid_size, block_size>>>(obs_GPU, beta_GPU, data_GPU, Log_like_GPU, *n, *p);
